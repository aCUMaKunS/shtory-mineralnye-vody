...
- `images/` — фото штор (jpg/webp), логотип/favicon (favicon.png) и фон hero (пока SVG-заглушка)
...
3. **Фото фона hero** — `images/hero-bg.svg` пока SVG-заглушка, замените на фото интерьера. Фото штор в карточках (`images/классические.jpg`, `римские.jpg`, `рулон.webp`, `блэкаут.jpg`, `тюль.jpg`) и логотип/favicon (`images/favicon.png`) уже реальные.
